...
def MdigProcessExample():
   ...
   MIL.MappControl(MIL.M_DEFAULT, MIL.M_ERROR, MIL.M_PRINT_DISABLE) # 오류 메세지 출력 안하도록 함
   for n in range(0, BUFFERING_SIZE_MAX): # 버퍼 사이즈 맥스 만큼 버퍼초기화
      ...
      if (MilGrabBufferList[n] != MIL.M_NULL):
         ...
         break
   MIL.MappControl(MIL.M_DEFAULT, MIL.M_ERROR, MIL.M_PRINT_ENABLE) # 오류 메세지 출력 하도록 함
